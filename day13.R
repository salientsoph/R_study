...
# id 기준으로 합치기
total <- inner_join(test1, test2, by = "id")  # 겹치는 id만
...
exam_new <- left_join(exam, name, by = "class")  # 일치하는 행이 쨌든 없어서.. 어떤 함수를 쓰던 다 똑같음
...
# base package = rbind(), cbind(), merge()  -> bind_rows등과 동일. but 더 느림.
...
boxplot(exam$math)
mean(exam$math, na.rm = T)  # 결측치 제외하고 math 평균 산출  # na가 있으면 평균으로 대체
# 평균으로 대체하기
...
boxplot(mpg$hwy, range=0)  # 무조건 최대, 최소값에 맞춰서 그린다. 이상치는 판단 x
